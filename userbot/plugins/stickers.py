...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "No Name Set Yet, Check @thedemonhub"
...
async def _(event):
    ...
    packname = f"KangSticker {userid}"
    packshortname = f"Kang{userid}"  # format: Uni_Borg_userid
    ...
    if is_a_s:
        ...
        if userid == 715462063:
            packshortname = "715462063_DemonHuB"
        else:
